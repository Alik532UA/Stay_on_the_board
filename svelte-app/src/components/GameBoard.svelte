<script>
  ...
  $: showMoves = $appState.settings?.showMoves;
  $: showBoard = typeof $appState.settings?.showBoard === 'boolean' ? $appState.settings.showBoard : true;

  $: console.log('[GameBoard] playerRow:', playerRow, 'playerCol:', playerCol, 'availableMoves:', availableMoves, 'blockedCells:', blockedCells);
  ...
  function isAvailable(/** @type {number} */ row, /** @type {number} */ col) {
    const result = availableMoves && availableMoves.some(move => move.row === row && move.col === col);
    console.log('[isAvailable] row:', row, 'col:', col, 'result:', result, 'availableMoves:', availableMoves);
    return result;
  }
...
<div class="game-board-container">
  <div class="game-board-top-row">
    <button class="main-menu-btn" title={mainMenuTitle} on:click={goToMainMenu}>
      <svg xmlns="http://www.w3.org/2000/svg" fill="#000000" viewBox="0 0 547.596 547.596" width="32" height="32" aria-label={mainMenuTitle} class="main-menu-btn-img">
        <g>
    ...
    <div class="board-size-dropdown-wrapper">
      <button class="board-size-dropdown-btn" on:click={toggleBoardSizeDropdown} aria-haspopup="listbox" aria-expanded={showBoardSizeDropdown}>
        ...
        <svg width="18" height="18" viewBox="0 0 24 24"><path d="M7 10l5 5 5-5z" fill="currentColor"/></svg>
      </button>
      ...
      <div class="game-board" style="--board-size: {boardSize}">
        {#each Array(boardSize) as _, rowIdx (rowIdx)}
          {#each Array(boardSize) as _, colIdx (colIdx)}
            <div
              class="board-cell {((rowIdx + colIdx) % 2 === 0) ? 'light' : 'dark'} {isBlocked(rowIdx, colIdx) ? 'blocked-cell' : ''}"
            >
