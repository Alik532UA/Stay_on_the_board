...
const initialBoard = createEmptyBoard(initialBoardSize);
const { row: initialRow, col: initialCol } = getRandomCell(initialBoardSize);
initialBoard[initialRow][initialCol] = 1; // Розміщуємо фігуру
const initialAvailableMoves = getAvailableMoves(initialRow, initialCol, initialBoardSize, {}, 0);
...
export const appState = writable(/** @type {AppState} */({
  ...
  blockedCells: [],
  availableMoves: initialAvailableMoves,
  ...
  gameId: 1,
  availableDistances: Array.from({ length: initialBoardSize - 1 }, (_, i) => i + 1),
  cellVisitCounts: {},
...
export function setDirection(dir) {
  appState.update(state => {
    ...
    // Якщо напрямок змінюється
    if (state.selectedDirection !== dir) {
      if (!state.distanceManuallySelected) {
        newDistance = 1;
        newManuallySelected = false;
      }
      // Якщо була вибрана вручну — зберігаємо
    } else {
      // Той самий напрямок: інкрементуємо, якщо не вручну
      if (!state.distanceManuallySelected) {
        if (!state.selectedDistance || state.selectedDistance >= maxDist) {
          newDistance = 1;
        } else {
...
// Видаляю stopReplay, autoPlayInterval, goToReplayStep, toggleAutoPlayForward, toggleAutoPlayBackward, toggleLimitReplayPath
