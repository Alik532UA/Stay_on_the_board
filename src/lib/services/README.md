...
### 1. Система нарахування очок (`scoreService.ts`)
Логіка залежить від режиму гри:

- **Проти комп'ютера**:
    - Базові бали (+1, +2, +3) залежно від видимості дошки та фігури.
    - Бонуси за відстань (>1) та перестрибування заблокованих клітинок.
    - Штраф (-2) за дзеркальний хід.
- **Багатокористувацькі режими**:
    - Система **"Справедливої перемоги"**: бали за раунд додаються до зафіксованих лише після завершення кола всіма гравцями.

### 2. Доступні ходи (`availableMovesService.ts`)
- Обчислюються на основі поточної позиції фігури та заблокованих клітинок.
- Відображаються як білі точки (`●`) лише під час ходу гравця.
- Пріоритет відображення: Підсвічений хід (вибраний) > Доступні ходи.
