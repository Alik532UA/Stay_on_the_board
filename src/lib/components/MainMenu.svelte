<script>
  ...
  function navigateTo(route) {
    if (route === '/game') {
      settingsStore.init(); // Гарантуємо коректну ініціалізацію налаштувань
  ...
  {:else}
    ...
    <div class="main-menu-subtitle">
      ...
      {#if import.meta.env.DEV && $appVersion}
        <span class="dev-version" on:click={() => { if (isDev) window.location.href = '/Stay_on_the_board/dev'; }} style="cursor: pointer;">dev v.{$appVersion}</span>
      {/if}
    </div>
    <div id="main-menu-buttons">
      ...
      <button class="modal-button secondary" on:click={() => navigateTo('/drag-and-drop-test')}>{$_('mainMenu.dragAndDropTest')}</button>
