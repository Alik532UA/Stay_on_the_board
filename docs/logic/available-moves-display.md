...
#### 1. Умови показу
- **Час показу**: Точки з'являються тільки після завершення анімації ходу VirtualPlayer або попереднього гравця.
...
### Гра з VirtualPlayer (`vsVirtualPlayer`)
- **За замовчуванням**: `showMoves = true`
- **Причина**: Допомагає гравцю планувати ходи проти VirtualPlayer
- **Автоматичне встановлення**: В `onGameStart()` для режиму `vsVirtualPlayer`
