...
- **Показ Ходу VirtualPlayer**: Після ходу VirtualPlayer показує хід VirtualPlayer до початку вибору гравцем (тобто до першої дії гравця — вибору напрямку або відстані, а не до підтвердження ходу)
...
- **Показ Ходу VirtualPlayer**: Окрема область показує ходи VirtualPlayer в режимі vsVirtualPlayer
...
### Огляд
Елемент `center-info` - це центральна кнопка в сітці напрямків, яка відображає поточний стан вибору ходу гравця або останній хід VirtualPlayer. Це ключовий візуальний індикатор, який надає користувачу інформацію про поточний вибір та можливості взаємодії.
...
- **Поведінка**: Очищає показ ходу VirtualPlayer, позначає відстань як вибрану вручну та оновлює стан

#### `showVirtualPlayerMove(direction, distance)`
- **Призначення**: Відображає хід VirtualPlayer в UI
- **Поведінка**: Оновлює як показ VirtualPlayer, так і центральну кнопку інформації
...
- **Стани**:
  ...
  - Хід VirtualPlayer: Показує останній хід VirtualPlayer з помаранчевим фоном
...
- `game.gameMode`: Показує/приховує елементи, специфічні для VirtualPlayer

### Інтеграція
- **Інтеграція з GameLogic**: Доступ через `window.gameControlsComponent` для показу ходу VirtualPlayer
...
### Огляд
GameModeModal — це модальне вікно, яке з'являється після натискання "Гра з VirtualPlayer" у головному меню. Дозволяє гравцю обрати режим: звичайний або з заблокованими клітинками.
...
### Логіка роботи
- Відображається поверх головного меню після натискання "Гра з VirtualPlayer"
