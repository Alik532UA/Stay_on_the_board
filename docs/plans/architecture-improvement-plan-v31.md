...
### Фаза 7: Оптимізація компонентів (низький пріоритет)

- [ ] **7.1. Розбиття `Modal.svelte` (29KB)**
  - [ ] Виділити `ModalHeader.svelte`
  - [ ] Виділити `ModalFooter.svelte`
  - [ ] Виділити контент-специфічні підкомпоненти

- [ ] **7.2. Розбиття `MainMenu.svelte` (19KB)**
  - [ ] Виділити `MenuSection.svelte`
  - [ ] Виділити `MenuButton.svelte`

- [ ] **7.3. Консолідація UI stores**
  - [ ] Об'єднати `uiStateStore` та `uiEffectsStore` (якщо доречно)

---

### Фаза 8: Консолідація таймерів (низький пріоритет)

- [ ] **8.1. Аналіз таймерів**
  - [ ] Проаналізувати відповідальності `timeService.ts`, `TimerService.ts`, `timerStore.ts`
  - [ ] Визначити чи є дублювання

- [ ] **8.2. Рефакторинг (за потреби)**
  - [ ] Об'єднати або чітко розділити відповідальності
