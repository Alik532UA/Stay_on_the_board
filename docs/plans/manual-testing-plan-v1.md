...
| :--- | :--- | :--- | :--- |
| TC-01 | **Коректний старт гри** | 1. Запустити гру проти VirtualPlayer. | На дошці у випадковій клітинці з'являється ферзь, а після цього плавно з'являються доступні ходи. Відсутні будь-які зайві анімації чи "фантомні" ходи. |
...
| TC-06 | **Синхронізація анімації (хід VirtualPlayer)** | 1. Зробити хід, щоб спровокувати хід VirtualPlayer. | 1. Анімація ходу гравця завершується. <br> 2. Витримується пауза. <br> 3. Анімація ходу VirtualPlayer завершується. <br> 4. Тільки після цього з'являються нові доступні ходи для гравця. |
| TC-07 | **Розділення UI та логіки (`center-info`)** | 1. Зробити хід гравцем. | Інформація про хід VirtualPlayer з'являється в `center-info` **миттєво** після ходу гравця, не чекаючи завершення візуальної анімації на дошці. |
| TC-08 | **Швидкі послідовні ходи** | 1. Спробувати дуже швидко зробити кілька ходів підряд, не чекаючи відповіді VirtualPlayer. | Система блокує можливість зробити новий хід, доки не завершиться повний цикл "хід гравця -> хід VirtualPlayer". Неможливо порушити черговість ходів. |
...
| TC-12 | **Повернення з "Перегляду запису"** | 1. Завершити будь-яку гру (локальну або проти VirtualPlayer). <br> 2. У модальному вікні результатів натиснути "Переглянути запис". <br> 3. Вийти з режиму перегляду. | Відбувається повернення до того самого модального вікна з результатами гри, яке було до перегляду запису. Нова гра не починається. |
...
| TC-20 | **Додавання VirtualPlayer в локальну гру** | 1. У налаштуваннях локальної гри додати одного гравця-людину та одного гравця-VirtualPlayer. <br> 2. Почати гру. | Коли настає черга гравця-VirtualPlayer, він робить хід автоматично, без участі користувача. Черга коректно переходить від людини до VirtualPlayer і назад. |

### Секція 5: Функціонал "Ходів немає"

| ID | Назва | Кроки для відтворення | Очікуваний результат |
| :--- | :--- | :--- | :--- |
| TC-21 | **"Ходів немає" (Гра проти VirtualPlayer) - Правильна заява** | 1. Запустити гру проти VirtualPlayer в режимі "Заблокованих клітинок". <br> 2. Довести гру до стану, коли у фігури немає жодного доступного ходу. <br> 3. Натиснути кнопку "Ходів немає". | З'являється модальне вікно "Блискучий аналіз!". Воно містить кнопки "Продовжити гру" та "Завершити (+N балів)". Бонус за "Ходів немає" коректно розрахований. |
| TC-22 | **"Ходів немає" (Гра проти VirtualPlayer) - Неправильна заява** | 1. Запустити гру проти VirtualPlayer. <br> 2. Переконатися, що є доступні ходи. <br> 3. Натиснути кнопку "Ходів немає". | З'являється модальне вікно "Помилкова заява!". У тексті вказано правильну кількість доступних ходів. Гра завершується, гравець програє. |
| TC-23 | **"Ходів немає" (Гра проти VirtualPlayer) - Продовження гри** | 1. Виконати кроки з TC-21. <br> 2. У модальному вікні "Блискучий аналіз!" натиснути "Продовжити гру". | Всі заблоковані клітинки на дошці візуально та логічно зникають. Гра продовжується, хід залишається у гравця. |
...
| ID | Назва | Кроки для відтворення | Очікуваний результат |
| :--- | :--- | :--- | :--- |
| TC-26 | **Вхід та вихід з реплею (Гра проти VirtualPlayer)** | 1. Завершити гру проти VirtualPlayer (програти). <br> 2. У модальному вікні "Гру завершено!" натиснути "Переглянути запис". <br> 3. На сторінці реплею натиснути кнопку "Назад". | Відбувається повернення на сторінку `/game/vs-virtual-player` з тим самим модальним вікном "Гру завершено!", яке було до перегляду. Рахунок та кнопки у вікні збережені. Нова гра не починається. |
...
| TC-28 | **Реплей з проміжного модального вікна ("Суперник у пастці!")** | 1. У грі проти VirtualPlayer довести до стану, коли у VirtualPlayer немає ходів. <br> 2. У модальному вікні "Суперник у пастці!" натиснути "Переглянути запис". <br> 3. На сторінці реплею натиснути "Назад". | Відбувається повернення до модального вікна "Суперник у пастці!" з усіма його кнопками ("Продовжити", "Завершити"). |
