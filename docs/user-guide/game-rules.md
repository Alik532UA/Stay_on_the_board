...
-   **Штрафні очки** нараховуються за "дзеркальні" ходи в режимі "Проти VirtualPlayer".
...
### Штрафні очки
-   **Штраф за дзеркальний хід (-2)**: У режимі "Проти VirtualPlayer", якщо VirtualPlayer робить хід на N клітинок в одному напрямку, а гравець одразу ж робить хід на N або менше клітинок у протилежному напрямку, застосовується штраф. Це запобігає повторюваним ходам туди-сюди. Цей штраф не застосовується в режимі "Заблоковані клітинки".
...
### Проти VirtualPlayer
-   Гравець змагається з VirtualPlayer.
-   Ходи VirtualPlayer визначаються випадковим алгоритмом, що робить його непередбачуваним.
-   Існує 1-секундна затримка перед ходом VirtualPlayer.
...
-   Показує останній хід VirtualPlayer для довідки гравця.

## 10. Стани гри та ігровий процес

Гра проходить через кілька станів:
1.  **Початковий стан**: Гра знаходиться в головному меню.
2.  **Вибір режиму гри**: З'являється модальне вікно для вибору між звичайним режимом та режимом "Заблоковані клітинки" (для гри проти VirtualPlayer).
...
7.  **Хід VirtualPlayer** (якщо застосовно): VirtualPlayer обирає та виконує свій хід.
...
Гра підтримує повне керування з клавіатури. Детальний опис усіх гарячих клавіш та їх налаштувань можна знайти в документі [Гарячі клавіші](./keyboard-shortcuts.md).
...
-   `makeVirtualPlayerMove()`: Виконує хід VirtualPlayer.
