...
- **Штрафні бали** за "дзеркальні" ходи (тільки в грі з VirtualPlayer)
...
- VirtualPlayer ходить після кожного ходу гравця
- Затримка 1 секунда між ходами VirtualPlayer
- Детальніше дивіться [virtual-player-logic.md](../development/virtual-player-logic.md)
...
- Показує хід VirtualPlayer після його виконання
...
### Проти VirtualPlayer
1. **Використовуйте передбачуваність**: VirtualPlayer ходить випадково
2. **Створюйте пастки**: заманюйте VirtualPlayer в невигідні позиції
...
### Обробка ходів
- **processPlayerMove()**: обробляє хід гравця та передає чергу VirtualPlayer
    - Після зміни currentPlayer на 2 (VirtualPlayer) одразу оновлюються доступні ходи для VirtualPlayer, щоб його хід був коректним
- **makeVirtualPlayerMove()**: виконує хід VirtualPlayer та передає чергу гравцю
- **executeMove()**: виконує хід (спільна функція для гравця та VirtualPlayer)
- **endGame()**: завершує гру

## Штрафні бали

- **Штрафні бали** нараховуються, якщо гравець робить гарантовано безпечний хід після ходу VirtualPlayer:
  - Якщо VirtualPlayer зробив хід на N клітинок у певному напрямку, а гравець одразу після цього робить хід у протилежному напрямку на N або меншу кількість клітинок, гравець отримує штраф.
...
- [virtual-player-logic.md](../development/virtual-player-logic.md) - Алгоритм VirtualPlayer 
...
- Чекбокси 'Показати фігуру' (showPiece) та 'Показувати доступні ходи' (showMoves) мають вмикатися ДО старту нової гри (до появи дошки).
