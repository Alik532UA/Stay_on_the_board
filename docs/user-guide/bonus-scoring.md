...
### Нарахування за ходи
- **+1 бал**: за хід з видимою дошкою та фігурою
- **+2 бали**: за хід з видимою дошкою, але прихованою фігурою  
...
- **НЕ включає базовий рахунок** (+1/+2/+3 за видимість дошки), бонуси за розмір дошки, режим блокування, "Ходів немає" та завершення гри
- **Використовує систему Fair Scoring** (розділення на зафіксовані бали та бали за поточне коло) - детальніше див. [fair-scoring.md](fair-scoring.md)
- **Визначення переможця**: переможцем стає гравець з найбільшою кількістю **зафіксованих балів** серед тих, хто не вийшов за межі дошки або не потрапив на заблоковану клітинку.

**Технічна реалізація:**
- `actualGameMode` (з `BaseGameMode.getModeName()`) визначає, чи нараховувати базові бали
- Для `actualGameMode === 'local'` або `'online'`: базові бали НЕ нараховуються
- Для `actualGameMode === 'virtual-player'`, `'training'`, `'timed'`: базові бали нараховуються
- **Важливо:** `settings.gameMode` містить пресет ('observer', 'beginner'), а НЕ фактичний режим гри!
