...
Проект дотримується принципу "Єдиного джерела правди" (SSoT), де весь стан гри зберігається в центральних сховищах Svelte (stores). Потік даних є односпрямованим, а відповідальність розділена між різними модулями (сховища, сервіси та компоненти).
...
4.  **Відладка:**
    ...
    *   **Фокусуйтеся на проблемі:** Під час виправлення помилки **тимчасово редагуйте файл `src/lib/services/logService.ts`**. Змініть булеві прапорці в об'єкті `logConfig`, щоб увімкнути лише ті групи логів, які стосуються проблеми (наприклад, `score`), а решту вимкнути. Це допоможе усунути інформаційний шум.
