...
test.describe('Модальне вікно підтвердження зміни розміру дошки', () => {
  ...
  test('1. Не повинно з\'являтися, якщо рахунок 0', { tag: '@done' }, async ({ page }) => {
  ...
  test('2. Повинно з\'являтися, якщо рахунок не 0', { tag: '@done' }, async ({ page }) => {
  ...
  test('3. Спочатку не з\'являється (рахунок 0), потім з\'являється (рахунок не 0)', { tag: '@done' }, async ({ page }) => {
    // Stage 1: Score is 0
    await expect(page.getByTestId('board-resize-confirm-modal')).not.toBeVisible();
    await page.getByTestId('increase-board-size-btn').click();
    await expect(page.getByTestId('board-resize-confirm-modal')).not.toBeVisible();
